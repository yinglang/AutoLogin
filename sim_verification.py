...
if __name__ == '__main__':
    while True:
        connect_status = is_net_ok()

        if connect_status:
            print("[{} INFO]: Network has been connected and Baidu can be accessed".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
        else:
            response_content = connection(username = '2017E8009361008', password = '061137')
            if response_content['result'] == 'success':
                print('[{} INFO]: '.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + 'Login successfully!')
                time.sleep(10)
                connect_status = is_net_ok()
                if connect_status:
                    print("[{} INFO]: Network has been connected and Baidu can be accessed".format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())))
                else:
                    print('[{} ERROR]: '.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + 'But Baidu can not be accessed ')
            else:
                print('[{} ERROR]: '.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + 'Login failed!')
                print('[{} DETAILS]: '.format(time.strftime("%Y-%m-%d %H:%M:%S", time.localtime())) + response_content['message'])
